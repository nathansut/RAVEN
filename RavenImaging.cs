...
public static class RavenImaging
{
    ...
    public static void ImgBackTrackThresholdAverage(int ImageHandle, int MinTreshold, int MaxTreshold, double Contribue) { } // dead code — never called
    ...
    /// <summary>Draw text on image. No-op (commented out in RAVEN).</summary>
    public static void ImgDrawText(int ImageHandle, int X, int Y, int FontName, int FontAttributes, int Orientation, int Color, string Data) { }
    ...
    // Pre-built LUT tables for Recogniform-compatible grayscale.
    // gray = LUT_R[r] + LUT_G[g] + LUT_B[b]
    private static readonly byte[] LUT_R = BuildGrayLut(0.30);
    private static readonly byte[] LUT_G = BuildGrayLut(0.59);
    private static readonly byte[] LUT_B = BuildGrayLut(0.11);

    private static byte[] BuildGrayLut(double weight)
    {
    ...
    public static (byte[] grayLut, byte[] grayAvg, int width, int height) LoadImageAsGrayscaleDual(string path)
    {
            ...
            temp.PixelFormat == PixelFormat.Format8bppIndexed)
        {
            ...
            for (int y = 0; y < h; y++)
            {
                ...
                int dstRow = y * w;
                for (int x = 0; x < w; x++)
                    g[dstRow + x] = bgr8[srcRow + x * 3]; // R==G==B for grayscale
            }
            return (g, g, w, h);
        }
    ...
    public static (byte[] gray, int width, int height) LoadImageAsGrayscale(string path)
    {
        ...
        Parallel.For(0, h, y =>
        {
            ...
            for (int x = 0; x < w; x++)
            {
                ...
                byte gv = bgr[off + 1];
                byte r = bgr[off + 2];
                gray[dstRow + x] = (byte)(LUT_R[r] + LUT_G[gv] + LUT_B[b]);
            }
        });
        return (gray, w, h);
    }
